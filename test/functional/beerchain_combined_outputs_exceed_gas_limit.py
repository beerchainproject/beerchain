...
class BeerchainCombinedOutputsExceedGasLimitTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        tx.vout = [
            CTxOut(0, CScript([b"\x04", CScriptNum(19998999), CScriptNum(BEERCHAIN_MIN_GAS_PRICE), hex_str_to_bytes("00"), hex_str_to_bytes(contract_address), OP_CALL])),
            CTxOut(0, CScript([b"\x04", CScriptNum(19998999), CScriptNum(BEERCHAIN_MIN_GAS_PRICE), hex_str_to_bytes("00"), hex_str_to_bytes(contract_address), OP_CALL])),
            CTxOut(0, CScript([b"\x04", CScriptNum(19998999), CScriptNum(BEERCHAIN_MIN_GAS_PRICE), hex_str_to_bytes("00"), hex_str_to_bytes(contract_address), OP_CALL]))
        ]
...
if __name__ == '__main__':
    BeerchainCombinedOutputsExceedGasLimitTest().main()
