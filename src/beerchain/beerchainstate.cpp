...
#include <beerchain/beerchainstate.h>

using namespace std;
using namespace dev;
using namespace dev::eth;

BeerchainState::BeerchainState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = BeerchainState::openDB(_path + "/beerchainDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

BeerchainState::BeerchainState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute BeerchainState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, BeerchainTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createBeerchainAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            beerchain::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
    ...
    if(voutLimit){
        ...
        return ResultExecute{ex, BeerchainTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs()), refund.vout.empty() ? CTransaction() : CTransaction(refund)};
    }else{
        return ResultExecute{res, BeerchainTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs()), tx ? *tx : CTransaction()};
    }
}

std::unordered_map<dev::Address, Vin> BeerchainState::vins() const // temp
{
...
void BeerchainState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* BeerchainState::vin(dev::Address const& _a) const
{
    return const_cast<BeerchainState*>(this)->vin(_a);
}

Vin* BeerchainState::vin(dev::Address const& _addr)
{
...
// void BeerchainState::commit(CommitBehaviour _commitBehaviour)
// {
...
void BeerchainState::kill(dev::Address _addr)
{
...
void BeerchainState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void BeerchainState::deleteAccounts(std::set<dev::Address>& addrs){
...
void BeerchainState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void BeerchainState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void BeerchainState::validateTransfersWithChangeLog(){
