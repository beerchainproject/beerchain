#include <beerchain/beerchainDGP.h>
...
void BeerchainDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool BeerchainDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule BeerchainDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t BeerchainDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t BeerchainDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t BeerchainDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t BeerchainDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool BeerchainDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void BeerchainDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void BeerchainDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void BeerchainDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void BeerchainDGP::createParamsInstance(){
...
dev::Address BeerchainDGP::getAddressForBlock(unsigned int blockHeight){
...
void BeerchainDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void BeerchainDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void BeerchainDGP::parseStorageOneUint64(uint64_t& value){
...
void BeerchainDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule BeerchainDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
