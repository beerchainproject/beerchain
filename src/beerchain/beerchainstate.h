...
#include <beerchain/beerchaintransaction.h>
...
class BeerchainTransactionReceipt: public dev::eth::TransactionReceipt {
public:
    BeerchainTransactionReceipt(dev::h256 const& state_root, dev::h256 const& utxo_root, dev::u256 const& gas_used, dev::eth::LogEntries const& log) : dev::eth::TransactionReceipt(state_root, gas_used, log), m_utxoRoot(utxo_root) {}
...
struct ResultExecute{
    ...
    BeerchainTransactionReceipt txRec;
    CTransaction tx;
};
...
class BeerchainState : public dev::eth::State {
    
public:

    BeerchainState();

    BeerchainState(dev::u256 const& _accountStartNonce, dev::OverlayDB const& _db, const std::string& _path, dev::eth::BaseState _bs = dev::eth::BaseState::PreExisting);

    ResultExecute execute(dev::eth::EnvInfo const& _envInfo, dev::eth::SealEngineFace const& _sealEngine, BeerchainTransaction const& _t, dev::eth::Permanence _p = dev::eth::Permanence::Committed, dev::eth::OnOpFunc const& _onOp = OnOpFunc());
    ...
    static const dev::Address createBeerchainAddress(dev::h256 hashTx, uint32_t voutNumber){
...
struct TemporaryState{
    std::unique_ptr<BeerchainState>& globalStateRef;
    dev::h256 oldHashStateRoot;
    dev::h256 oldHashUTXORoot;

    TemporaryState(std::unique_ptr<BeerchainState>& _globalStateRef) : 
...
public:
    ...
    CondensingTX(BeerchainState* _state, const std::vector<TransferInfo>& _transfers, const BeerchainTransaction& _transaction, std::set<dev::Address> _deleteAddresses = std::set<dev::Address>()) : transfers(_transfers), deleteAddresses(_deleteAddresses), transaction(_transaction), state(_state){}
...
private:
    ...
    const BeerchainTransaction& transaction;

    BeerchainState* state;
