    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                pindexNew->hashStateRoot  = diskindex.hashStateRoot; // beerchain
                pindexNew->hashUTXORoot   = diskindex.hashUTXORoot; // beerchain
                pindexNew->nStakeModifier = diskindex.nStakeModifier;
                pindexNew->prevoutStake   = diskindex.prevoutStake;
                pindexNew->vchBlockSig    = diskindex.vchBlockSig; // beerchain
