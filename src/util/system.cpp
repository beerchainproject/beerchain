...
const char * const BITCOIN_CONF_FILENAME = "beerchain.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Beerchain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Beerchain
    // Mac: ~/Library/Application Support/Beerchain
    // Unix: ~/.beerchain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Beerchain";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Beerchain";
#else
    // Unix
    return pathRet / ".beerchain";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("Beerchain Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Beerchain Core Developers";
    }
