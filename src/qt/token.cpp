...
bool ToHash160(const std::string& strBeerchainAddress, std::string& strHash160)
{
    CTxDestination beerchainAddress = DecodeDestination(strBeerchainAddress);
    if(!IsValidDestination(beerchainAddress))
        return false;
    const PKHash * keyid = boost::get<PKHash>(&beerchainAddress);
    if(keyid){
...
bool ToBeerchainAddress(const std::string& strHash160, std::string& strBeerchainAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    PKHash keyid(key);
    CTxDestination beerchainAddress = keyid;
    if(IsValidDestination(beerchainAddress)){
        strBeerchainAddress = EncodeDestination(beerchainAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToBeerchainAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToBeerchainAddress(tokenEvent.receiver, tokenEvent.receiver);
