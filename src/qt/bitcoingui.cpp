...
#include <qt/beerchainversionchecker.h>
    ...
    m_network_style(networkStyle)
{
    ...
    beerchainVersionChecker = new BeerchainVersionChecker(this);
#ifdef ENABLE_WALLET
    if(enableWallet) {
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Beerchain address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and beerchain: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Beerchain addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Beerchain addresses"));
    ...
    openAction->setStatusTip(tr("Open a beerchain: URI or payment request"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Beerchain command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && beerchainVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(BEERCHAIN_RELEASES, BEERCHAIN_RELEASES);
            QString message(tr("New version of Beerchain wallet is available on the Beerchain source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Beerchain network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
