    ...
    m_results(1)
{
    ...
    ui->labelAmount->setToolTip(tr("The amount in BEERCHAIN to send. Default = 0."));
    ui->labelSenderAddress->setToolTip(tr("The beerchain address that will be used as sender."));
...
void SendToContract::setModel(WalletModel *_model)
{
    ...
    // update the display unit, to not use the default ("BEERCHAIN")
    updateDisplayUnit();
}
...
void SendToContract::on_gasInfoChanged(quint64 blockGasLimit, quint64 minGasPrice, quint64 nGasPrice)
{
    ...
    ui->labelGasPrice->setToolTip(tr("Gas price: BEERCHAIN price per gas unit. Default = %1, Min = %2").arg(QString::fromStdString(FormatMoney(DEFAULT_GAS_PRICE))).arg(QString::fromStdString(FormatMoney(minGasPrice))));
