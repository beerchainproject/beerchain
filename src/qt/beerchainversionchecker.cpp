...
#define paternVersion "beerchain-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

BeerchainVersionChecker::BeerchainVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

BeerchainVersionChecker::~BeerchainVersionChecker()
{

}

bool BeerchainVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> BeerchainVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(BEERCHAIN_RELEASES)));
    QEventLoop event;
...
Version BeerchainVersionChecker::getMaxReleaseVersion()
{
