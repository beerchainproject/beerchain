...
#if defined(NDEBUG)
#error "Beerchain cannot be compiled without assertions."
#endif
...
std::unique_ptr<BeerchainState> globalState;
...
bool fAddressIndex = false; // beerchain
#endif
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (tx.HasCreateOrCall()) {
        ...
        BeerchainDGP beerchainDGP(globalState.get(), fGettingValuesDGP);
        uint64_t minGasPrice = beerchainDGP.getMinGasPrice(::ChainActive().Tip()->nHeight + 1);
        uint64_t blockGasLimit = beerchainDGP.getBlockGasLimit(::ChainActive().Tip()->nHeight + 1);
        size_t count = 0;
        ...
        BeerchainTxConverter converter(tx, NULL, NULL, contractflags);
        ExtractBeerchainTX resultConverter;
        if (!converter.extractionBeerchainTransactions(resultConverter)) {
        ...
        std::vector<BeerchainTransaction> beerchainTransactions = resultConverter.first;
        std::vector<EthTransactionParams> beerchainETP = resultConverter.second;

        dev::u256 sumGas = dev::u256(0);
        dev::u256 gasAllTxs = dev::u256(0);
        for (BeerchainTransaction beerchainTransaction : beerchainTransactions) {
            sumGas += beerchainTransaction.gas() * beerchainTransaction.gasPrice();
            ...
            if (txMinGasPrice != 0) {
                txMinGasPrice = std::min(txMinGasPrice, beerchainTransaction.gasPrice());
            } else {
                txMinGasPrice = beerchainTransaction.gasPrice();
            }
            VersionVM v = beerchainTransaction.getVersion();
            if (v.format != 0)
            ...
            if (beerchainTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
            ...
            if (beerchainTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
            ...
            if (beerchainTransaction.gas() > UINT32_MAX)
            ...
            gasAllTxs += beerchainTransaction.gas();
            ...
            if (v.rootVM != 0 && (uint64_t)beerchainTransaction.gasPrice() < minGasPrice)
...
bool MemPoolAccept::Finalize(ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : allConflicting) {
        LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s BEERCHAIN additional fees, %d delta bytes\n",
...
static bool AcceptToMemoryPoolWithTime(const CChainParams& chainparams, CTxMemPool& pool, CValidationState& state, const CTransactionRef& tx, bool* pfMissingInputs, int64_t nAcceptTime, std::list<CTransactionRef>* plTxnReplaced, bool bypass_limits, const CAmount nAbsurdFee, bool test_accept, bool rawTx = false) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransactionRef& tx, bool* pfMissingInputs, std::list<CTransactionRef>* plTxnReplaced, bool bypass_limits, const CAmount nAbsurdFee, bool test_accept, bool rawTx)
{
...
{
    //
    // Premine 1% of 21,000,000,000 BEER
    //
    if (nHeight <= 10)
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot));    // beerchain
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // beerchain
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit)
{
    ...
    BeerchainTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());


    ByteCodeExec exec(block, std::vector<BeerchainTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
bool CheckReward(const CBlock& block, CValidationState& state, int nHeight, const Consensus::Params& consensusParams, CAmount nFees, CAmount gasRefunds, CAmount nActualStakeReward, const std::vector<CTxOut>& vouts)
{
    ...
    if (block.IsProofOfWork()) {
        ...
        if (block.vtx[offset]->GetValueOut() > blockReward)
            return state.Invalid(ValidationInvalidReason::CONSENSUS, error("CheckReward(): coinbase pays too much (actual=%d vs limit=%d)", block.vtx[offset]->GetValueOut(), blockReward), REJECT_INVALID, "bad-cb-amount");
    } else {
        // Check full reward
        CAmount blockReward = nFees + GetBlockSubsidy(nHeight, consensusParams);
        if (nActualStakeReward > blockReward)
            return state.Invalid(ValidationInvalidReason::CONSENSUS, error("CheckReward(): coinstake pays too much (actual=%d vs limit=%d)", nActualStakeReward, blockReward), REJECT_INVALID, "bad-cs-amount");
...
{
    boost::filesystem::path beerchainDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if (fIsVMlogFile) {
        ss << ",";
    } else {
        std::ofstream file(beerchainDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(beerchainDir.string(), std::ios::in | std::ios::out);
...
{
    for (BeerchainTransaction& tx : txs) {
        //validate VM version
        ...
        if (!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())) {
            ...
            result.push_back(ResultExecute{execRes, BeerchainTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries()), CTransaction()});
            continue;
        }
...
bool BeerchainTxConverter::extractionBeerchainTransactions(ExtractBeerchainTX& beerchaintx)
{
    ...
    std::vector<BeerchainTransaction> resultTX;
    ...
    beerchaintx = std::make_pair(resultTX, resultETP);
    return true;
}

bool BeerchainTxConverter::receiveStack(const CScript& scriptPubKey)
{
    sender = false;
...
bool BeerchainTxConverter::parseEthTXParams(EthTransactionParams& params)
{
    try {
...
BeerchainTransaction BeerchainTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut)
{
    BeerchainTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL) {
        txEth = BeerchainTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    } else {
        txEth = BeerchainTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t BeerchainTxConverter::correctedStackSize(size_t size)
{
...
bool CChainState::ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // beerchain
    BeerchainDGP beerchainDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setBeerchainSchedule(beerchainDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight + 1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1)));
    uint32_t sizeBlockDGP = beerchainDGP.getBlockSize(pindex->nHeight + (pindex->nHeight + 1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = beerchainDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight + 1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = beerchainDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight + 1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // beerchain
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (tx.HasCreateOrCall() && !hasOpSpend) {
            ...
            BeerchainTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractBeerchainTX resultConvertBeerchainTX;
            if (!convert.extractionBeerchainTransactions(resultConvertBeerchainTX)) {
            ...
            if (!CheckMinGasPrice(resultConvertBeerchainTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertBeerchainTX.first, blockGasLimit, pindex->pprev);
            ...
            for (BeerchainTransaction& qtx : resultConvertBeerchainTX.first) {
            ...
            if (fLogEvents && !fJustCheck) {
                for (size_t k = 0; k < resultConvertBeerchainTX.first.size(); k++) {
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertBeerchainTX.first[k].from(),
                        resultConvertBeerchainTX.first[k].to(),
                        ...
                        resultConvertBeerchainTX.first[k].getNVout()});
                }
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash());    // beerchain
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // beerchain
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot);    // beerchain
            globalState->setRootUTXO(oldHashUTXORoot); // beerchain
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView* coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    for (pindex = ::ChainActive().Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // beerchain
        uint32_t sizeBlockDGP = beerchainDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Beerchain PoS we need to sync this after every block to ensure txdb is populated for
