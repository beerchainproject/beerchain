...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setBeerchainSchedule(beerchainDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = beerchainDGP.getBlockSize(nHeight);
    minGasPrice = beerchainDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = beerchainDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    BeerchainTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractBeerchainTX resultConverter;
    if(!convert.extractionBeerchainTransactions(resultConverter)){
    ...
    for(BeerchainTransaction beerchainTransaction : beerchainTransactions){
        txGas += beerchainTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + beerchainTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(beerchainTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, beerchainTransactions, hardBlockGasLimit, ::ChainActive().Tip());
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "beerchainstake";
...
void StakeBeerchains(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
