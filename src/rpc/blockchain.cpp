...
double GetEstimatedAnnualROI()
{
    ...
    if(networkWeight > 0)
    {
        // Formula: 10000 * 2880 blocks/day * 365 days * subsidy) / Network Weight
        result = 10512000000 * subsidy / networkWeight;// Beerchain
    }
...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    result.pushKV("hashStateRoot", blockindex->hashStateRoot.GetHex()); // beerchain
    result.pushKV("hashUTXORoot", blockindex->hashUTXORoot.GetHex()); // beerchain
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* tip, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("hashStateRoot", block.hashStateRoot.GetHex()); // beerchain
    result.pushKV("hashUTXORoot", block.hashUTXORoot.GetHex()); // beerchain
...
UniValue transactionReceiptToJSON(const BeerchainTransactionReceipt& txRec)
{
    UniValue result(UniValue::VOBJ);
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() >= 3){
        CTxDestination beerchainSenderAddress = DecodeDestination(request.params[2].get_str());
        if (IsValidDestination(beerchainSenderAddress)) {
            const PKHash *keyid = boost::get<PKHash>(&beerchainSenderAddress);
...
UniValue gettxout(const JSONRPCRequest& request)
{
            ...
            "     \"addresses\" : [          (array of string) array of beerchain addresses\n"
