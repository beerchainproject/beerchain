...
extern std::unique_ptr<BeerchainState> globalState;
...
inline void initState(){
    ...
    const std::string dirBeerchain = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<BeerchainState>(new BeerchainState(dev::u256(0), BeerchainState::openDB(dirBeerchain, hashDB, dev::WithExisting::Trust), dirBeerchain + "/beerchainDB", dev::eth::BaseState::Empty));
...
inline dev::Address createBeerchainAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline BeerchainTransaction createBeerchainTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    BeerchainTransaction txEth;
    if(recipient == dev::Address()){
        txEth = BeerchainTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = BeerchainTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<BeerchainTransaction> txs){
