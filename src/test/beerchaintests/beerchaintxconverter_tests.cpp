...
void checkResult(bool isCreation, std::vector<BeerchainTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    BeerchainTxConverter converter(transaction, NULL);
    ExtractBeerchainTX beerchainTx;
    BOOST_CHECK(converter.extractionBeerchainTransactions(beerchainTx));
    std::vector<BeerchainTransaction> result = beerchainTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(beerchaintxconverter_tests, TestingSetup)
