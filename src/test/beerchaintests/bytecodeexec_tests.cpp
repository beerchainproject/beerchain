...
#include <beerchaintests/test_utils.h>
...
BOOST_AUTO_TEST_CASE(bytecodeexec_txs_empty){
    initState();
    std::vector<BeerchainTransaction> txs;
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract){
    initState();
    BeerchainTransaction txEth = createBeerchainTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<BeerchainTransaction> txs(1, txEth);
    auto result = executeBC(txs);
    
    std::vector<dev::Address> addrs = {createBeerchainAddress(txs[0].getHashWith(), txs[0].getNVout())};
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract_OutOfGasIntrinsic){
    initState();
    BeerchainTransaction txEth = createBeerchainTransaction(CODE[0], 0, dev::u256(100), dev::u256(1), HASHTX, dev::Address());
    std::vector<BeerchainTransaction> txs(1, txEth);
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract_OutOfGas){
    initState();
    BeerchainTransaction txEth = createBeerchainTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<BeerchainTransaction> txs(1, txEth);
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_OutOfGasIntrinsic_create_contract_normal_create_contract){
    ...
    for(size_t i = 0; i < 10; i++){
        ...
        BeerchainTransaction txEth = createBeerchainTransaction(CODE[0], 0, gasLimit, dev::u256(1), hash, dev::Address());
        newAddressGen.push_back(createBeerchainAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_OutOfGas_create_contract_normal_create_contract){
    ...
    for(size_t i = 0; i < 10; i++){
        ...
        BeerchainTransaction txEth = createBeerchainTransaction(code, 0, GASLIMIT, dev::u256(1), hash, dev::Address());
        newAddressGen.push_back(createBeerchainAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract_many){
    ...
    for(size_t i = 0; i < 130; i++){
        BeerchainTransaction txEth = createBeerchainTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), hash, dev::Address(), i);
        newAddressGen.push_back(createBeerchainAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer){
    initState();
    BeerchainTransaction txEthCreate = createBeerchainTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<BeerchainTransaction> txsCreate(1, txEthCreate);
    executeBC(txsCreate);
    std::vector<dev::Address> addrs = {createBeerchainAddress(txsCreate[0].getHashWith(), txsCreate[0].getNVout())};
    BeerchainTransaction txEthCall = createBeerchainTransaction(ParseHex("00"), 1300, GASLIMIT, dev::u256(1), HASHTX, addrs[0]);
    std::vector<BeerchainTransaction> txsCall(1, txEthCall);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer_OutOfGasIntrinsic_return_value){
    ...
    std::vector<BeerchainTransaction> txsCreate(1, txEthCreate);
    executeBC(txsCreate);
    dev::Address newAddress(createBeerchainAddress(txsCreate[0].getHashWith(), txsCreate[0].getNVout()));
    BeerchainTransaction txEthCall = createBeerchainTransaction(ParseHex("00"), 1300, dev::u256(1), dev::u256(1), HASHTX, newAddress);
    std::vector<BeerchainTransaction> txsCall(1, txEthCall);
    auto result = executeBC(txsCall);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer_OutOfGas_return_value){
    initState();
    BeerchainTransaction txEthCreate = createBeerchainTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<BeerchainTransaction> txsCreate(1, txEthCreate);
    executeBC(txsCreate);
    dev::Address newAddress(createBeerchainAddress(txsCreate[0].getHashWith(), txsCreate[0].getNVout()));
    BeerchainTransaction txEthCall = createBeerchainTransaction(ParseHex("00"), 1300, GASLIMIT, dev::u256(1), HASHTX, newAddress);
    std::vector<BeerchainTransaction> txsCall(1, txEthCall);
    auto result = executeBC(txsCall);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer_many){
    ...
    executeBC(txs);
    std::vector<BeerchainTransaction> txsCall;
    std::vector<dev::Address> addrs;
    valtype codeCall(ParseHex("00"));
    dev::h256 hashCall(HASHTX);
    for(size_t i = 0; i < txs.size(); i++){
        BeerchainTransaction txEthCall = createBeerchainTransaction(codeCall, 1300, GASLIMIT, dev::u256(1), hash, newAddressGen[i], i);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_OutOfGas_transfer_many_return_value){
    ...
    dev::h256 hash(HASHTX);
    for(size_t i = 0; i < 130; i++){
        BeerchainTransaction txEth = createBeerchainTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), hash, dev::Address(), i);
        newAddressGen.push_back(createBeerchainAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
    ...
    dev::h256 hashCall(HASHTX);
    for(size_t i = 0; i < txs.size(); i++){
        BeerchainTransaction txEthCall = createBeerchainTransaction(codeCall, 1300, GASLIMIT, dev::u256(1), hashCall, newAddressGen[i], i);
        txsCall.push_back(txEthCall);
        addrs.push_back(txEthCall.receiveAddress());
        ++hashCall;
    }
...
BOOST_AUTO_TEST_CASE(bytecodeexec_suicide){
    initState();
    std::vector<BeerchainTransaction> txsCreate;
    std::vector<dev::Address> newAddresses;
    dev::h256 hash(HASHTX);
    for(size_t i = 0; i < 10; i++){
        valtype code = i == 0 ? CODE[0] : CODE[4];
        dev::u256 value = i == 0 ? dev::u256(0) : dev::u256(13);
        BeerchainTransaction txEthCreate = createBeerchainTransaction(code, 0, GASLIMIT, dev::u256(1), hash, dev::Address(), i);
        txsCreate.push_back(txEthCreate);

        BeerchainTransaction txEthSendValue;
        if(i != 0){
            txEthSendValue = createBeerchainTransaction(valtype(), value, GASLIMIT, dev::u256(1), hash, createBeerchainAddress(txEthCreate.getHashWith(), txEthCreate.getNVout()), i);
            txsCreate.push_back(txEthSendValue);
        }

        newAddresses.push_back(createBeerchainAddress(txEthCreate.getHashWith(), txEthCreate.getNVout()));
        ++hash;
    }
    ...
    for(size_t i = 0; i < newAddresses.size() - 1; i++){
        BeerchainTransaction txEthCall = createBeerchainTransaction(codeCall, 0, GASLIMIT, dev::u256(1), HASHTX, newAddresses[i + 1]);
        txsCall.push_back(txEthCall);
        addrs.push_back(txEthCall.receiveAddress());
    }
    auto result = executeBC(txsCall);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_contract_create_contracts){
    initState();
    BeerchainTransaction txEthCreate = createBeerchainTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<BeerchainTransaction> txs(1, txEthCreate);
    executeBC(txs);
    std::vector<BeerchainTransaction> txsCall;
    valtype codeCall(ParseHex("3f811b80"));
    dev::Address newAddress(createBeerchainAddress(txEthCreate.getHashWith(), txEthCreate.getNVout()));
    std::vector<dev::Address> addrs;
    for(size_t i = 0; i < 20; i++){
        BeerchainTransaction txEthCall = createBeerchainTransaction(codeCall, 0, GASLIMIT, dev::u256(1), HASHTX, newAddress, i);
        txsCall.push_back(txEthCall);
        addrs.push_back(txEthCall.receiveAddress());
    }
    auto result = executeBC(txsCall);
