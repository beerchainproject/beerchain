...
void genesisLoading(){
    ...
    dev::eth::ChainParams cp((chainparams.EVMGenesisInfo(dev::eth::Network::beerchainMainNetwork, 999)));
...
BOOST_AUTO_TEST_CASE(checking_returndata_opcode_after_fork){
    ...
    txs.push_back(createBeerchainTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    txs.push_back(createBeerchainTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
    executeBC(txs);

    // Call upgrade to
    dev::Address proxy = createBeerchainAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<BeerchainTransaction> txsCall;
    txsCall.push_back(createBeerchainTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    executeBC(txsCall);

    // Call mint
    std::vector<BeerchainTransaction> txsMint;
    txsMint.push_back(createBeerchainTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    auto result = executeBC(txsMint);
    BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);

    // Call balance of
    std::vector<BeerchainTransaction> txsbalance;
    txsbalance.push_back(createBeerchainTransaction(ParseHex("70a082310000000000000000000000000101010101010101010101010101010101010101"), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
...
BOOST_AUTO_TEST_CASE(checking_constantinople_after_fork){
    ...
    // Create contract
    std::vector<BeerchainTransaction> txs;
    txs.push_back(createBeerchainTransaction(CODE[4], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    executeBC(txs);

    // Call is it constantinople
    dev::Address proxy = createBeerchainAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<BeerchainTransaction> txIsItConstantinople;
    txIsItConstantinople.push_back(createBeerchainTransaction(CODE[5], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
