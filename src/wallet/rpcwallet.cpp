    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"getnewaddress",
                "\nReturns a new Beerchain address for receiving payments.\n"
                ...
                RPCResult{
            "\"address\"    (string) The new beerchain address\n"
                },
                RPCExamples{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"getrawchangeaddress",
                "\nReturns a new Beerchain address, for receiving change.\n"
...
static UniValue setlabel(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"setlabel",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The beerchain address to be associated with a label."},
    ...
    if (!IsValidDestination(dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Beerchain address");
    }
...
static UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"sendtoaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The beerchain address to send to."},
            ...
            "                             The recipient will receive less BEER than you enter in the amount field."},
            ...
            "                             dirty if they have previously been used in a transaction."},
                    {"senderaddress", RPCArg::Type::STR, RPCArg::Optional::OMITTED_NAMED_ARG, "The beerchain address that will be used to send money from."},
    ...
    senderAddress = DecodeDestination(request.params[9].get_str());
        if (!IsValidDestination(senderAddress))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Beerchain address to send from");
        else
            fHasSender=true;
    }
...
static UniValue createcontract(const JSONRPCRequest& request){
    ...
    uint64_t blockGasLimit = beerchainDGP.getBlockGasLimit(::ChainActive().Height());
    uint64_t minGasPrice = CAmount(beerchainDGP.getMinGasPrice(::ChainActive().Height()));
    CAmount nGasPrice = (minGasPrice>DEFAULT_GAS_PRICE)?minGasPrice:DEFAULT_GAS_PRICE;
                ...
                HelpRequiringPassphrase(pwallet) + "\n",
                {
                    ...
                    {"gasPrice", RPCArg::Type::AMOUNT, RPCArg::Optional::OMITTED, "gasPrice BEERCHAIN price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)},
                    {"senderaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::OMITTED, "The beerchain address that will be used to create the contract."},
    ...
    CAmount nGasPrice = (minGasPrice>DEFAULT_GAS_PRICE)?minGasPrice:DEFAULT_GAS_PRICE;
                ...
                RPCHelpMan{"sendtocontract",
                    ...
                    HelpRequiringPassphrase(pwallet) + "\n",
                    {
                        ...
                        {"gasPrice", RPCArg::Type::AMOUNT, RPCArg::Optional::OMITTED, "gasPrice Beerchain price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)},
                        {"senderaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::OMITTED, "The beerchain address that will be used as sender."},
                        {"broadcast", RPCArg::Type::BOOL, RPCArg::Optional::OMITTED, "Whether to broadcast the transaction or not."},
                        {"changeToSender", RPCArg::Type::BOOL, RPCArg::Optional::OMITTED, "Return the change to the sender."},
                    },
                    RPCResult{
...
static UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The beerchain address\n"
        ...
        return NullUniValue;
            ...
            RPCHelpMan{"signmessage",
                "\nSign a message with the private key of an address" +
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The beerchain address to use for the private key."},
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"getreceivedbyaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The beerchain address for transactions."},
...
static UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    RPCHelpMan{"sendmany",
                    ...
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    ...
                    {"amounts", RPCArg::Type::OBJ, RPCArg::Optional::NO, "A json object with addresses and amounts",
                        {
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "The beerchain address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value"},
                        },
                    },
            ...
            "                           Those recipients will receive less BEER than you enter in their corresponding amount field.\n"
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Beerchain address: ") + name_);
        }
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"addmultisigaddress",
                ...
                "Each key is a Beerchain address or hex-encoded public key.\n"
                "This functionality is only intended for use with non-watchonly addresses.\n"
                "See `importaddress` for watchonly p2sh address support.\n"
                "If 'label' is specified, assign address to that label.\n",
                {
                    {"nrequired", RPCArg::Type::NUM, RPCArg::Optional::NO, "The number of required signatures out of the n keys or addresses."},
                    {"keys", RPCArg::Type::ARR, RPCArg::Optional::NO, "A json array of beerchain addresses or hex-encoded public keys",
                        {
                            {"key", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "beerchain address or hex-encoded public key"},
                        },
                        },
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            "[\n"
            "  {\n"
            "    \"address\":\"address\",    (string) The beerchain address of the transaction.\n"
...
static UniValue gettransaction(const JSONRPCRequest& request_)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            "  \"details\" : [\n"
            "    {\n"
            "      \"address\" : \"address\",          (string) The beerchain address involved in the transaction\n"
            "      \"category\" :                      (string) The transaction category.\n"
            "                   \"send\"                  Transactions sent.\n"
            "                   \"receive\"               Non-coinbase transactions received.\n"
            "                   \"generate\"              Coinbase transactions received with more than 100 confirmations.\n"
    ...
    if (!shouldWaitConf) {
        ...
        if(!request.req)
            throw JSONRPCError(RPC_INTERNAL_ERROR, "No HTTP connection. Waitconf is available from beerchain-cli, not beerchain-qt");
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"walletpassphrase",
                ...
                "This is needed prior to performing transactions related to private keys such as sending BEERCHAIN and staking\n"
            "\nNote:\n"
...
static UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending beerchain\n"
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"lockunspent",
                ...
                "A locked transaction output will not be chosen by automatic coin selection, when spending BEER.\n"
...
static UniValue loadwallet(const JSONRPCRequest& request)
{
            RPCHelpMan{"loadwallet",
                ...
                "\nNote that all wallet command-line options used when starting beerchaind will be"
...
static UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    RPCHelpMan{
                ...
                "Optionally filter to only include txouts paid to specified addresses.\n",
                {
                    ...
                    {"addresses", RPCArg::Type::ARR, /* default */ "empty array", "A json array of beerchain addresses to filter",
                        {
                            {"address", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "beerchain address"},
                        },
                    },
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the beerchain address\n"
    ...
    if (!request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Beerchain address: ") + input.get_str());
            }
    ...
    if (!options.isNull()) {
      if (options.type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid beerchain address");
            }
    ...
    RPCHelpMan{"fundrawtransaction",
                ...
                "Only pay-to-pubkey, multisig, and P2SH versions thereof are currently supported for watch-only\n",
                {
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "for backward compatibility: passing in a true instead of an object will result in {\"includeWatching\":true}",
                        {
                            {"changeAddress", RPCArg::Type::STR, /* default */ "pool address", "The beerchain address to receive the change"},
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"getaddressinfo",
                "\nReturn information about the given beerchain address. Some information requires the address\n"
                "to be in the wallet.\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The beerchain address to get the information of."},
                },
                RPCResult{
            "{\n"
            "  \"address\" : \"address\",        (string) The beerchain address validated\n"
                ...
                "Implements the Creator and Updater roles.\n",
                {
                    ...
                    {"outputs", RPCArg::Type::ARR, RPCArg::Optional::NO, "a json array with outputs (key-value pairs), where none of the keys are duplicated.\n"
                            ...
                            "                             accepted as second parameter.",
                        {
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                                {
                                    {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "A key-value pair. The key (string) is the beerchain address, the value (float or string) is the amount in " + CURRENCY_UNIT + ""},
                                },
                                },
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                                {
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "",
                        {
                            {"changeAddress", RPCArg::Type::STR_HEX, /* default */ "pool address", "The beerchain address to receive the change"},
                            {"changePosition", RPCArg::Type::NUM, /* default */ "random", "The index of the change output"},
                            {"change_type", RPCArg::Type::STR, /* default */ "set by -changetype", "The output type to use. Only valid if changeAddress is not specified. Options are \"legacy\", \"p2sh-segwit\", and \"bech32\"."},
                            {"includeWatching", RPCArg::Type::BOOL, /* default */ "true for watch-only wallets, otherwise false", "Also select inputs which are watch only"},
                            {"lockUnspents", RPCArg::Type::BOOL, /* default */ "false", "Lock selected unspent outputs"},
