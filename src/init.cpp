 ...
 */
static const char* BITCOIN_PID_FILENAME = "beerchaind.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    util::ThreadRename("beerchain-shutoff");
...
void SetupServerArgs()
{
    ...
    const auto testnetBaseParams = CreateBaseChainParams(CBaseChainParams::MAIN);
    const auto regtestBaseParams = CreateBaseChainParams(CBaseChainParams::MAIN);
    const auto defaultChainParams = CreateChainParams(CBaseChainParams::MAIN);
    const auto testnetChainParams = CreateChainParams(CBaseChainParams::MAIN);
    const auto regtestChainParams = CreateChainParams(CBaseChainParams::MAIN);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/beerchainproject/beerchain>";
    const std::string URL_WEBSITE = "<https://beerchain.network>";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(GetDataDir() / "stateBeerchain");
...
bool AppInitParameterInteraction()
{
    ...
    if (gArgs.IsArgSet("-minimubeerchainwork")) {
        const std::string minChainWorkStr = gArgs.GetArg("-minimubeerchainwork", "");
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            fs::path beerchainStateDir = GetDataDir() / "stateBeerchain";
            bool fStatus = fs::exists(beerchainStateDir);
            const std::string dirBeerchain(beerchainStateDir.string());
            const dev::h256 hashDB(dev::sha3(dev::rlp("")));
            dev::eth::BaseState existsBeerchainstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
            globalState = std::unique_ptr<BeerchainState>(new BeerchainState(dev::u256(0), BeerchainState::openDB(dirBeerchain, hashDB, dev::WithExisting::Trust), dirBeerchain, existsBeerchainstate));
            dev::eth::ChainParams cp((chainparams.EVMGenesisInfo(dev::eth::Network::beerchainMainNetwork)));
            globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

            pstorageresult.reset(new StorageResults(beerchainStateDir.string()));
            if (fReset) {
