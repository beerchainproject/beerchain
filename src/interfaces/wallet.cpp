...
public:
    ...
    bool isUnspentAddress(const std::string &beerchainAddress) override
    {
        auto locked_chain = m_wallet->chain().lock();
        LOCK(m_wallet->cs_wallet);
        ...
        for (const COutput& out : vecOutputs)
        {
            ...
            if(fValidAddress && EncodeDestination(address) == beerchainAddress && out.tx->tx->vout[out.i].nValue)
            {
                return true;
            }
        }
