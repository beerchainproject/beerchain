...
#include <beerchain/storageresults.h>
...
using ExtractBeerchainTX = std::pair<std::vector<BeerchainTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // beerchain
...
class BeerchainTxConverter{

public:

    BeerchainTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL, unsigned int flags = SCRIPT_EXEC_BYTE_CODE) : txBit(tx), view(v), blockTransactions(blockTxs), sender(false), nFlags(flags){}

    bool extractionBeerchainTransactions(ExtractBeerchainTX& beerchainTx);
...
private:
    ...
    BeerchainTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<BeerchainTransaction> _txs, const uint64_t _blockGasLimit, CBlockIndex* _pindex) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit), pindex(_pindex) {}
