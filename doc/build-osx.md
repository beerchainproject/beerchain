...
1. Clone the Beerchain Core source code:
    ```shell
...
2.  Build Beerchain Core:
    ...
    Configure and build the headless Beerchain Core binaries as well as the GUI (if Qt is found).
...
## Running
Beerchain Core is now available at `./src/beerchaind`

Before running, you may create an empty configuration file:
```shell
mkdir -p "/Users/${USER}/Library/Application Support/Beerchain"

touch "/Users/${USER}/Library/Application Support/Beerchain/beerchain.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Beerchain/beerchain.conf"
```

The first time you run beerchaind, it will start downloading the blockchain. This process could
take many hours, or even days on slower than average systems.

You can monitor the download process by looking at the debug.log file:
```shell
tail -f $HOME/Library/Application\ Support/Beerchain/debug.log
```

## Other commands:
```shell
./src/beerchaind -daemon      # Starts the beerchain daemon.
./src/beerchain-cli --help    # Outputs a list of command-line options.
./src/beerchain-cli help      # Outputs a list of RPC commands when the daemon is running.
```
