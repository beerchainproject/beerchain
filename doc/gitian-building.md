Gitian building
================
*Setup instructions for a Gitian build of Beerchain Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Beerchain
...
- [Building Beerchain Core](#building-beerchain-core)
...
Clone the git repositories for beerchain and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/beerchainproject/beerchain --recursive
git clone https://github.com/beerchainproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/beerchain/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the beerchain repository under 'Fetch and create inputs' to install sources which require
...
To build Beerchain Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the beerchain repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/beerchain/.git/
...
For example:
```bash
URL=https://github.com/laanwj/beerchain.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit beerchain=${COMMIT} --url beerchain=${URL} ../beerchain/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit beerchain=${COMMIT} --url beerchain=${URL} ../beerchain/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit beerchain=${COMMIT} --url beerchain=${URL} ../beerchain/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-beerchain:
./bin/gbuild --commit beerchain=${COMMIT},cpp-eth-beerchain=${ETHCOMMIT} --url beerchain=${URL},cpp-eth-beerchain=${ETHURL} ../beerchain/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the beerchain git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../beerchain/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/beerchainproject/beerchain-detached-sigs.git

BTCPATH=/some/root/path/beerchain
SIGPATH=/some/root/path/beerchain-detached-sigs

./bin/gbuild --url beerchain=${BTCPATH},signature=${SIGPATH} ../beerchain/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/beerchain-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/beerchain-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/beerchain-osx-build.assert
```
...
[beerchainproject/gitian.sigs](https://github.com/beerchainproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@beerchain.org) and he will commit them.
